...
col_title=[r'Title',r'Art.NO', r'Size', r'Stock', r'Price', r'LinkURL', r'ShopURL']
file_name='data.xls'
...
def get_input( queue_shop_url, filename='shop.txt'):
    ...
    while True:
        ...
        if len(line)==0:
            ...
            m=re.search(r'http://(\w+).taobao.com', line)
            if m is not None:
...
#            print page_number
            for page_id in xrange(1, page_number):    
                ...
                try:
                    ...
                    continue
#                print html
                for m in re.finditer(r'<a[\s]+target="_blank"[\s]+href="http://item.taobao.com/auction/(.+).htm">', html):
                    t_url = 'http://item.taobao.com/auction/%s.htm'%m.group(1)
                    #g_mutex.acquire()
...
#                print threading.currentThread()
            ...
            for m in re.finditer(r'<li data-value="\d*:\d*"><a href="#"><span>(.*)</span></a></li>', html):
                ...
                #ff.setValue(ff.row, ff.col, m.group(1))
                #ff.addCol()
            if len(size)==0:
                size.append('null')
            ...
            f_mutex.acquire()
            for i_size, i_stock in zip(size, stock):
                ff.addRow()
                ...
                ff.addCol()
                ff.setValue(ff.row, ff.col, price)
                ff.addCol()
